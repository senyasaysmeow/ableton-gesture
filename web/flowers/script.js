...
(function () {
  ...
  class Flower {
    constructor() {
      ...
      this.tx = clamp(
        this.x + rand(-0.2 * spread, 0.2 * spread),
        -0.2 * W,
        1.2 * W,
      );
      this.ty = clamp(
        this.y + rand(-0.2 * spread, 0.2 * spread),
        -0.2 * H,
        1.2 * H,
      );
    ...
    queueHueTowardsPalette() {
      ...
      let bestHue = this.hue;
      let best = Infinity;
      for (const h of hues) {
        const d = Math.abs(((h - this.hue + 540) % 360) - 180);
        if (d < best) {
          best = d;
          bestHue = h;
        }
    ...
    draw(ctx) {
      const a = this.alpha * 0.9;
      if (a <= 0.01) return;
      const petals = this.petals;
      const r = this.radius * state.sizeScale;
      ctx.save();
      ctx.globalAlpha = a;
      ...
      for (let i = 0; i < petals; i++) {
        ...
        ctx.save();
        ctx.rotate(ang);
        const pr = r * (i % 2 ? 1.15 : 0.95);
        const wr = pr * this.petalFatness;
        const wobble =
          Math.sin((performance.now() / 1000) * this.wobbleFreq + i) *
          (this.wobbleAmp * state.sizeScale);
        const grad = ctx.createLinearGradient(0, 0, pr, 0);
        grad.addColorStop(
          0,
          hsl(this.hue, this.sat, Math.min(85, this.light + 20), 0.95),
        );
        grad.addColorStop(
          0.7,
          hsl(this.hue, this.sat, Math.max(15, this.light - 5), 0.95),
        );
        grad.addColorStop(
          1,
          hsl(this.hue, this.sat, Math.max(10, this.light - 20), 0.92),
        );
        ctx.fillStyle = grad;
        ctx.beginPath();
        const round = this.petalRoundness;
        ctx.moveTo(0, 0);
        ctx.bezierCurveTo(
          wr * 0.25,
          -wr * round + wobble,
      ...
      if (this.doubleLayer) {
        const innerPetals = Math.max(5, Math.round(petals * 0.7));
        ctx.save();
        ctx.globalAlpha *= 0.9;
        ctx.rotate(Math.PI / innerPetals);
        for (let i = 0; i < innerPetals; i++) {
          const ang = (i / innerPetals) * Math.PI * 2;
          ctx.save();
          ctx.rotate(ang);
          const pr = this.radius * state.sizeScale * 0.65;
          const wr = pr * (this.petalFatness * 0.8);
          const wobble =
            Math.sin((performance.now() / 1000) * (this.wobbleFreq * 1.2) + i) *
            (this.wobbleAmp * 0.6 * state.sizeScale);
          const grad = ctx.createLinearGradient(0, 0, pr, 0);
          grad.addColorStop(
            0,
            hsl(this.hue, this.sat, Math.min(88, this.light + 28), 0.95),
          );
          grad.addColorStop(
            1,
            hsl(this.hue, this.sat, Math.max(15, this.light - 10), 0.9),
          );
          ctx.fillStyle = grad;
          ctx.beginPath();
          const round = Math.min(0.9, Math.max(0.2, this.petalRoundness + 0.1));
          ctx.moveTo(0, 0);
          ctx.bezierCurveTo(
            wr * 0.25,
            -wr * round + wobble,
      ...
      ctx.fill();
      const dots = 16;
      const ringR = coreR * 1.25;
      for (let i = 0; i < dots; i++) {
        const ang = (i / dots) * Math.PI * 2 + this.rotation * 0.2;
        const dx = Math.cos(ang) * ringR;
        const dy = Math.sin(ang) * ringR;
        ctx.beginPath();
        ctx.fillStyle = hsl((this.hue + 45) % 360, 90, 65, 0.9);
        ctx.arc(
          dx,
          dy,
          Math.max(0.8, this.radius * state.sizeScale * 0.03),
          0,
          Math.PI * 2,
        );
        ctx.fill();
  ...
  function animate(ts) {
    ...
    const dt = Math.max(
      0.0005,
      Math.min(0.03, dtRaw * 0.75 + (state._prevDt || dtRaw) * 0.25),
    );
    state._prevDt = dt;
    state.lastTime = ts;
    ...
    state.spawnTimer -= dt;
    while (state.flowers.length < target && state.spawnTimer <= 0) {
      spawnFlower();
      state.spawnTimer += rand(0.05, 0.15);
    }
    if (state.flowers.length > target) {
      const excess = state.flowers.length - target;
      for (let i = 0; i < excess; i++) {
        const f = state.flowers[i];
        f.maxLife = Math.min(f.maxLife, f.life + rand(0.6, 1.8));
      }
    }
    for (let i = state.flowers.length - 1; i >= 0; i--) {
      ...
      f.step(dt);
      f.draw(ctx);
      if (f.dead) state.flowers.splice(i, 1);
    }
  ...
  const control = {
    setRotationSpeed(v) {
      state.rotationSpeed = Math.max(0, Math.min(6.0, v));
    },
    setSizeScale(s) {
      state.sizeScale = Math.max(0.2, Math.min(3.0, s));
    },
    ...
    setVisible(on) {
      state.visibilityTarget = on ? 1 : 0;
    },
    setVisibleLevel(v) {
      state.visibilityTarget = clamp(v, 0, 1);
    },
    setFadeSpeed(rate) {
      state.fadeRate = Math.max(0.5, Math.min(20, rate));
    },
  };
