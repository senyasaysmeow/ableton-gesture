...
(function () {
  ...
  function buildPetalInstances() {
    ...
    for (const f of state.flowers) {
      ...
      for (let layer = 0; layer < layerCount; layer++) {
        const petCount =
          layer == 0 ? petals : Math.max(5, Math.round(petals * 0.7));
        const baseR =
          (layer == 0 ? f.radius : f.radius * 0.65) * state.sizeScale;
        for (let i = 0; i < petCount; i++) {
          const ang =
            (i / petCount) * twoPI + (layer == 1 ? Math.PI / petCount : 0);
  ...
  function buildCoreInstances() {
    ...
    for (const f of state.flowers) {
      ...
      const hue = f.hue;
      const sat = Math.min(1.0, f.sat + 0.1);
      const light = Math.max(0.0, f.light - 0.1);
  ...
  function buildDotInstances() {
    ...
    for (const f of state.flowers) {
      ...
      const hue = f.hue;
      const sat = Math.min(1.0, f.sat + 0.2);
      const light = Math.min(1.0, f.light + 0.1);
  ...
  function buildShadowInstances() {
    ...
    for (const f of state.flowers) {
      ...
      const hue = f.hue;
      const sat = Math.max(0.0, f.sat - 0.2);
      const light = 0.2;
  ...
  function animate(ts) {
    ...
    // spawn/retire
    state.spawnTimer -= dt;
    while (state.flowers.length < state.targetCount && state.spawnTimer <= 0) {
      spawnFlower();
    ...
    for (let idx = 0; idx < state.flowers.length; idx++) {
      ...
      if (instanceCount > 0) {
        ...
        gl.bufferSubData(
          gl.ARRAY_BUFFER,
          0,
          arr.subarray(0, instanceCount * 8),
        );
  ...
  const control = {
    ...
    setPaletteFromHue(base) {
      ...
      for (const f of state.flowers) {
        let best = f.hue,
          dMin = 1e9;
        for (const ph of state.hues) {
          const d = Math.abs(((ph - f.hue + 540) % 360) - 180);
          if (d < dMin) {
            dMin = d;
            best = ph;
          }
